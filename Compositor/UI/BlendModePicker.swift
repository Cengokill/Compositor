...
struct BlendModePicker: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSMenuDelegate {
        ...
        func menu(_ menu: NSMenu, willHighlight item: NSMenuItem?) {
            ...
            // the canvas never flashes back to the layer's previous mode. Tags stay
            // valid after titles are translated; matching on the displayed title would not.
            guard let mode = item.flatMap({ menuItem in
                LayerBlendMode.allCases.indices.contains(menuItem.tag) ? LayerBlendMode.allCases[menuItem.tag] : nil
            }) else { return }
