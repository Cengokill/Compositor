...
extension EditorSession {
    ...
    static func attributedText(_ style: LayerTextStyle) -> NSMutableAttributedString {
        ...
        for run in style.fontRuns ?? [] where run.length > 0 && run.location >= 0 && NSMaxRange(NSRange(location: run.location, length: run.length)) <= string.length {
        ...
        for run in style.colorRuns ?? [] where run.length > 0 && run.location >= 0 && NSMaxRange(NSRange(location: run.location, length: run.length)) <= string.length {
