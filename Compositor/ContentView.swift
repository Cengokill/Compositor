...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onChange(of: session.selectionAmountOperation) { _, operation in
            if let operation {
                ...
                selectionAmountPanel.show(title: localizedString(operation.rawValue + " Selection"),
                    content: SelectionAmountSheet(session: session, operation: operation))
            } else { selectionAmountPanel.close() }
        }
        .onChange(of: session.filterEdit == nil) { _, closed in syncFilterPanel(closed: closed) }
    ...
    /// Kept out of `body` so SwiftUI's type checker can finish; that expression is already near its limit.
    @ViewBuilder
    ...
    private func syncLevelsPanel(closed: Bool) {
        if closed { levelsPanel.close() }
        else {
            levelsPanel.onClose = { session.cancelLevels() }
            levelsPanel.show(title: localizedString("Levels"), content: LevelsSheet(session: session))
        }
    }

    private func syncAdjustmentPanel(closed: Bool) {
        if closed { adjustmentPanel.close() }
        else {
            adjustmentPanel.onClose = { session.cancelHueSaturation() }
            adjustmentPanel.show(title: localizedString("Hue/Saturation"), content: HueSaturationSheet(session: session))
        }
    }

    private func syncFilterPanel(closed: Bool) {
        if closed { filterPanel.close() }
        else {
            filterPanel.onClose = { session.cancelFilter() }
            let title = session.filterEdit.map { $0.kind.localizedName } ?? localizedString("Filter")
    ...
    private var toolStatusMessage: String {
        ...
        case .shape:
            ...
            case .line:
                return localizedString("Drag to draw a shape on a new layer · Shift 45° · Option from center · Shift-U or Tab for the next shape · Escape cancel · Space to pan")
            case .rectangle:
                return localizedString("Drag to draw a shape on a new layer · Shift square · Option from center · Shift-U or Tab for the next shape · Escape cancel · Space to pan")
            case .ellipse:
                return localizedString("Drag to draw a shape on a new layer · Shift circle · Option from center · Shift-U or Tab for the next shape · Escape cancel · Space to pan")
            }
        case .gradient:
